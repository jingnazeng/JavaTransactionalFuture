...
//	Vacation.potentialPara += finishTiming - startTiming;
	...
	int n;
	int queriesPerTx = numQuery / (Operation.numberOfTransactionalFuture + 1);
...
//	List<ParallelTask<Boolean>> workers = new ArrayList<ParallelTask<Boolean>>();
	...
	for (n = 0; n < numQuery - queriesPerTx; n += queriesPerTx) {
	  ...
	  AsyncOperation continuation = new AsyncOperation(numQuery - queriesPerTx,numQuery);
	  isFoundinContinuation = continuation.call();

//	for (Future<Boolean> future : results) {
	  for (int i = 0; i < results.size();i++) {
		  Future<Boolean> future = results.get(i);
		  try {
			  //The commented par is what parallel nesting do, break at the first found sibling,
			  //but this is unfair comparision to sequential version, therefore we change
			  if (isFoundinContinuation || future.get()) {
				  isFound = true;
				  break;
			  }
//			  isFound = isFoundinContinuation;
//			  while(!future.isDone()){}
//			  isFound = future.get();
